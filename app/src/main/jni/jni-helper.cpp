...
extern "C" {
JNIEXPORT jint JNICALL Java_com_github_cgg_clasha_JniHelper_sigkill(JNIEnv *env, jobject thiz,
                                                                     jint pid) {
...
JNIEXPORT jint JNICALL Java_com_github_cgg_clasha_JniHelper_sigterm(JNIEnv *env, jobject thiz,
                                                                     jobject process) {
...
JNIEXPORT jobject JNICALL
        Java_com_github_cgg_clasha_JniHelper_getExitValue(JNIEnv *env, jobject thiz,
                                                           jobject process) {
    if (!env->IsInstanceOf(process, ProcessImpl)) {
        THROW(env, "java/lang/ClassCastException",
                   "Unsupported process object. Only java.lang.ProcessManager$ProcessImpl is accepted.");
...
JNIEXPORT jobject JNICALL
        Java_com_github_cgg_clasha_JniHelper_getExitValueMutex(JNIEnv *env, jobject thiz,
                                                                jobject process) {
    if (!env->IsInstanceOf(process, ProcessImpl)) {
        THROW(env, "java/lang/ClassCastException",
                   "Unsupported process object. Only java.lang.ProcessManager$ProcessImpl is accepted.");
...
JNIEXPORT void JNICALL Java_com_github_cgg_clasha_JniHelper_close(JNIEnv *env, jobject thiz,
                                                                   jint fd) {
    close(fd);
}

JNIEXPORT jint JNICALL
        Java_com_github_cgg_clasha_JniHelper_sendFd(JNIEnv *env, jobject thiz, jint tun_fd,
                                                     jstring path) {
    int fd;
